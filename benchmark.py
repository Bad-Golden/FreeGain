...
canceller + gate, plus the frequency shifter in feedback mode) runs on this
machine, for each "Room echo tail" setting in both modes.
...
from audio_engine import FEEDBACK_SHIFT_HZ, TAIL_OPTIONS_MS
...
def main():
    ...
    for mode in ("feedback mode", "spill mode"):
        print(f"{mode}:")
        for tail_ms in TAIL_OPTIONS_MS:
            taps = int(tail_ms / 1000 * SAMPLE_RATE)
            if mode == "feedback mode":
                canceller = PEMFDAF(taps, sample_rate=SAMPLE_RATE)
                shifter = FrequencyShifter(FEEDBACK_SHIFT_HZ, SAMPLE_RATE, BLOCK)
            else:
                canceller, shifter = PartitionedFDAF(taps), None
            gate = SimpleGate(sample_rate=SAMPLE_RATE)
            worst = 0.0
            start = time.perf_counter()
            for i in range(0, n, BLOCK):
                t0 = time.perf_counter()
                out = gate.process_block(canceller.process(reference[i:i + BLOCK], mic[i:i + BLOCK]))
