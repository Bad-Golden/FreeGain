"""
console_profiles.py

Behringer/Midas XAir and X32 use the same general OSC address scheme
(both are documented in Behringer's own OSC protocol PDFs) but differ in:
  - UDP port: X32/M32 family = 10023, XAir family = 10024
    (confirmed against Behringer's own X32-OSC protocol documentation)
  - Channel count: XAir models max out at 16-18 input channels depending
    on model (X18, XR18, XR16, XR12); X32 supports up to 32.

This is just configuration data -- the actual OSC address patterns
("/ch/01/mix/fader" etc.) are identical between the two families, so
MixerOSCClient itself doesn't need to change, just which port/channel
count it's configured with.
"""

CONSOLE_PROFILES = {
    "xair": {
        "label": "Behringer XAir (X18 / XR18 / XR16 / XR12)",
        "port": 10024,
        "max_channels": 18,  # XR18's ceiling; X18/XR16/XR12 have fewer --
                              # channel dropdown just won't populate names
                              # for channels your specific model doesn't have.
    },
    "x32": {
        "label": "Behringer X32 family",
        "port": 10023,
        "max_channels": 32,
    },
    "m32": {
        "label": "Midas M32",
        # Same protocol family as X32 -- confirmed in Behringer's own
        # X32-OSC documentation ("X32 & M32 represent a family of digital
        # mixers... using a communication protocol that is compatible to
        # standard OSC"). Identical port and channel count to X32.
        "port": 10023,
        "max_channels": 32,
    },
}

DEFAULT_PROFILE = "xair"
